    ...
    Column {
        ...
        ViewStateContainer(
            ...
            LazyColumn(state = listState, verticalArrangement = Arrangement.spacedBy(8.dp)) {
                items(forumList, key = { "${it.nick}_${it.time.time}" }) { entry ->
                    ForumListItem(entry = entry)
                }
            }
        }
    }
...
@Composable
fun ForumListItem(entry: ForumEntry) {
    ElevatedCard(
        modifier = Modifier
            .fillMaxWidth()
        ...
        Row(modifier = Modifier.padding(16.dp)) {
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text(text = entry.nick, style = MaterialTheme.typography.titleSmall)
                Text(
                    text = "${entry.forum} - ${formatDate(entry.time)}",
